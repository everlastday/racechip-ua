...
class UN_Model{
    ...
    if ( isset( $_COOKIE[ 'Captcha' ] ) ) {
      ...
      if ( md5( 'HDBHAYYEJKPWIKJHDD' . $params[ 'easycaptcha' ] . $_SERVER[ 'REMOTE_ADDR' ] . $time ) != $hash ) {

        $errors []= __('Код с картинки указан неверно. Попробуйте еще раз, или обновите картинку.', 'usernoise');
      } elseif( ( time() - 5 * 60) > $time ) {
        $errors []= __('Время истекло. Пожалуйста обновите картинку и введите новий код', 'usernoise');

      } else {

      }
    } else {
      $errors []= __('Пожалуйста, введите код с картинки', 'usernoise');
    }
	...
	public function send_admin_message($id, $params){
		...
		$message = sprintf(__('Новое сообщение добавлено в раздел обратной связи. <br />Сообщение можно просмотреть здесь: <a href="%s">%s</a>.'),
			//$type,
