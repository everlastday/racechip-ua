...
class UN_Settings {
	...
	public function __construct(){
		...
		$this->options = new Plugin_Options_Framework_0_2_1(USERNOISE_DIR, 
			array(), 
	...
	public function set_options(){
		...
		$general_options = array(
			...
			array('type' => 'checkbox', 'name' => UN_DISABLE_ON_MOBILES,
				'title' => __('Hide on mobile devices', 'usernoise'), 
				'label' => __('Hide on mobile devices', 'usernoise'),
				'default' => '1'),
			array('type' => 'checkbox', 'name' => UN_SHOW_POWERED_BY,
				'title' => __('Show Powered by', 'usernoise'),
				'label' => __('I like Usernoise and want to show <em>"Powered by Usernoise"</em>  link at the modal window.', 'usernoise'),
				'default' => '1'),
			array('type' => 'checkbox', 'name' => UN_LOAD_IN_FOOTER, 
				'title' => __('Load Usernoise in footer', 'usernoise'),
				'label' => __('Load Usernoise in footer', 'usernoise'),
				'legend' => __('Load Usernoise javascripts in footer. Sligtly increases page loading speed, but may be incompatible with some themes.', 'usernoise')),
			array('type' => 'select', 'name' => UN_USE_FONT, 'title' => __('Font', 'usernoise'),
				'values' => $this->h->array2options($fonts, __("Site default font", 'usernoise')),
				'default' => ''),
			array('type' => 'section', 'title' => __('Button', 'usernoise')),
			...
			array('type' => 'select', 'name' => UN_FEEDBACK_BUTTON_POSITION,
				...
				'default' => 'left'));
			$form_options = array(
			array('type' => 'tab', 'title' => __('Feedback form', 'usernoise')),
			...
			array('type' => 'tab', 'title' => __('Admin notifications', 'usernoise')),
			array('type' => 'checkbox', 'name' => UN_ADMIN_NOTIFY_ON_FEEDBACK,
				'title' => __('New feedback received', 'usernoise'),
				'label' => __('New feedback received', 'usernoise'),
				'default' => '1',
				'legend' =>
	...
	public function _all_in_one_promo($options){
		$options []= array('type' => 'custom', 
			...
			'html' => "Check out <a href='http://codecanyon.net/item/all-in-one-email-for-wordpress/1290390?ref=karevn'>All in One Email plugin</a>. It adds email options missing in WordPress natively.");
		return $options;
	}
	public function _pof_before_page_title($namespace){
		if ($this->options->namespace != $namespace)
			return;
		$this->h->link_to($this->h->_img(usernoise_url('/images/all-in-one-email.png')), 
